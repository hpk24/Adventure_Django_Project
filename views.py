...
from myapp1.models import bookingData
...
def userpackages(request):
    orders = packagebookingData.objects.filter(user=1)
    msg = 'Order Placed!'
...
def BookingAdventure(request):
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid() :
            ...
            # clientdata = client.objects.get(Q(first_name=order.user))
           # EmailData(order.user,order.People,clientdata.email_id)
            orders = bookingData.objects.all()
            msg = 'Order Placed!'
            return render(request, 'myapp1/bookingRecords.html',{'orders':orders})
    else:
...
def ForgetPassword(request):
    try:
        if request.method == 'POST':
            ...
            print(f"\n{token}\n")
            profile_obj, created = Profile.objects.get_or_create(user=user_obj, defaults={'forget_password_token' : f'{token}'})
            profile_obj.save()
